...
/**
 * Cliente HTTP interno (machine-to-machine) pro NimbusCore - resolve nome/username de usuários e
 * pede o dump do banco do NimbusCore pro backup sob demanda (ver
 * com.nimbusdesk.common.backup.BackupService). Mesmo papel do NimbusCoreInternalClient do
...
public class NimbusCoreInternalClient implements RemoteUserLookupClient {
   ...
   *  /internal/users/permissions no NimbusCore) - pra resolver destinatários de notificação a
  ...
  /** Pede ao NimbusCore um dump (pg_dump formato custom) do próprio banco dele - usado pelo
   *  backup sob demanda (com.nimbusdesk.common.backup.BackupService). O NimbusCore não expõe suas
   *  credenciais de banco pra fora; quem sabe rodar o pg_dump é ele mesmo, aqui só recebemos os
   *  bytes prontos (ver GET /internal/backup/database no NimbusCore). NÃO degrada silenciosamente
